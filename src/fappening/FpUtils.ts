...
import {fpCheckFpTblExist, fpCreateWithCheckExist} from "../@utils/PsDbUtils.js";
...
async function findDetails(context: BrowserContext, entry: FpCelebrityDetailEntry, fpResourceList: FpCelebrityDetailEntry[]) {
...
export async function findCelebrityDetails(celebrityList: FpCelebrityDetailEntry[], context: BrowserContext) {
    ...
    for (let entry of celebrityList) {
        ...
        //     continue;
        // }
        if ((await fpCheckFpTblExist(entry.postId)) !== null) {
            printLogSync(LogLevel.CONSOLE, `postId:${entry.postId}, name:${entry.name} has not handled yet, skip...`)
            continue;
        }
        ...
        let fpResourceListTemp = await findDetails(context, entry, fpResourceList);
